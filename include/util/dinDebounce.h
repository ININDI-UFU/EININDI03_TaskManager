...
class DigitalDebounce {
  public:
     ...
     */
    DigitalDebounce(uint8_t pin, unsigned long debounceDelay = 50, CallbackFunc callback = nullptr);

    /**
     * @brief Inicializa o pino com o modo especificado.
...
DigitalDebounce::DigitalDebounce(uint8_t pin, unsigned long debounceDelay, CallbackFunc callback)
  : _pin(pin),
...
void DigitalDebounce::begin(uint8_t mode) {
  pinMode(_pin, mode);
  // Inicializa os estados com a leitura atual do pino
  _currentState = digitalRead(_pin);
  _stableState = _currentState;
}

void DigitalDebounce::update() {
  // Lê o pino
...
bool DigitalDebounce::state() {
  return _stableState;
}

void DigitalDebounce::setCallback(CallbackFunc callback) {
  _callback = callback;
}