...
inline bool startDisplay(Display_c *disp, const uint8_t &SDA = 0, const uint8_t &SCL = 0) { return disp->start(SDA, SCL); }
bool Display_c::start(const uint8_t &SDA, const uint8_t &SCL)
{
  // SSD1306_SWITCHCAPVCC = generate display voltage from 3.3V internally
  if (SDA != 0 && SCL != 0)
    Wire.setPins(SDA, SCL); // Set the I2C pins before begin
